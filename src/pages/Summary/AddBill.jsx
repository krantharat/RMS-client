...
const CreateBill = ({ onClose }) => {
  ...
  const handleSubmit = (e) => {
    ...
    billCounter += 1; // Increment the global bill counter
    onClose();
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
      <div className="relative bg-white p-5 border-gray-200 rounded-2xl shadow-sm w-11/12 md:w-6/12 lg:w-6/12 h-auto">
        <div className="flex justify-center items-center">
          <h3 className="text-2xl font-bold">Create Bill</h3>
        </div>
        <div className="p-5 bg-white h-96 overflow-y-auto border border-gray-300 mt-3">
          <form className="grid grid-cols-1 gap-4" onSubmit={handleSubmit}>
                ...
                >
                  ...
                  {menuData.map(item => (
                    <option key={item.id} value={item.id}>
                      {item.menu} - ${item.price.toFixed(2)}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="flex justify-center mt-2">
              <button
                type="button"
                className="w-20 bg-blue-500 text-white font-medium capitalize border-0 rounded-3xl ml-5 p-2 hover:bg-blue-700 transition duration-300"
